...
const Plans = () => {
  ...
  return (
    <section id="planos" className="py-28 md:py-36 px-6 md:px-16">
      <div className="container mx-auto">
        ...
        >
          <span className="inline-flex items-center gap-2 text-sm font-medium text-gold border border-gold-subtle rounded-full px-5 py-2">
            ...
            return (
              ...
              >
                ...
                <div className="p-8 pb-0">
                  {plan.popular && (
                    <span className="inline-block text-xs font-semibold bg-primary-foreground/20 text-primary-foreground px-3 py-1 rounded-full mb-4">
                      Recomendado
                    </span>
                  )}
                  ...
                  {/* Highlight */}
                  <div className={`mt-4 mb-6 text-sm font-medium px-4 py-2.5 rounded-xl ${
                    plan.popular 
                ...
                <div className="px-8">
                  <ul className="space-y-3">
                    {plan.previewFeatures.map((f) => (
                      <li key={f} className={`flex items-start gap-3 text-sm ${plan.popular ? "text-primary-foreground/90" : "text-foreground"}`}>
                        <Check size={15} strokeWidth={2} className={`mt-0.5 flex-shrink-0 ${plan.popular ? "text-primary-foreground" : "text-gold"}`} />
                        {f}
                      </li>
                    ))}
                  </ul>
                      ...
                      <div className="px-8 pt-3">
                        ...
                        <ul className="space-y-3">
                          {plan.expandedFeatures.map((f) => (
                            <li key={f} className={`flex items-start gap-3 text-sm ${plan.popular ? "text-primary-foreground/80" : "text-muted-foreground"}`}>
                              <Check size={15} strokeWidth={2} className={`mt-0.5 flex-shrink-0 ${plan.popular ? "text-primary-foreground/60" : "text-gold/60"}`} />
                              {f}
                            </li>
                          ))}
                        </ul>
                ...
                <button
                  ...
                  className={`flex items-center justify-center gap-1.5 text-xs font-medium py-3 mt-4 mx-8 rounded-xl transition-colors ${
                    plan.popular
                ...
                <div className="p-8 pt-4">
                  <a
                    href="#contato"
                    className={`w-full inline-flex items-center justify-center gap-2 rounded-full py-4 text-sm font-semibold transition-all ${
                      plan.popular
